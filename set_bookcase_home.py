#!/usr/bin/env python3
"""Set home fields and clear decay/weight overrides on placed bookcases."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "placed bookcase dynamic records, and remove overloadedWeight."
        )
    )
    parser.add_argument(
        "data_dir",
...
def process_block(
    ...
    def finish_record(record_end: int) -> None:
        ...
        new_record, changed = process_record(record, stats, target_types, block_index, sample_limit)
        if changed:
...
def scan_dynamic(
    ...
    for block_index, entry in enumerate(entries):
        ...
        new_block_data, changed = process_block(block_data, stats, target_types, block_index, sample_limit)
        if changed:
...
def print_report(
    ...
    print(
        f"Skipped already complete with decayCount 0 and no overloadedWeight: "
    ...
    if stats.target_types and top:
        ...
        if len(stats.target_types) > top:
            ...
            print(f"{remaining_count:7d} {remaining_changed:7d} ... {len(stats.target_types) - top} more item type(s)")
        print()

    if stats.changed_records == 0:
        print("No target bookcase records need home, decayCount, overloadedWeight, or stat changes.")
        return
